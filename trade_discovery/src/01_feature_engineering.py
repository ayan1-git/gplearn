...
class OptimizedOrderBlockEngine:
    def __init__(self, internal_lookback=5, swing_lookback=20, atr_multiplier=0.5, max_obs=5, iou_threshold=0.85, missing_value_fill=5.0):
        self.int_lb = internal_lookback; self.swg_lb = swing_lookback; self.atr_mult = atr_multiplier
        self.max_obs = max_obs; self.iou_threshold = iou_threshold; self.missing_fill = missing_value_fill
    ...
    def get_pivot_flags(self, series: pd.Series, lookback: int, is_high: bool) -> np.ndarray:
        ...
        left_bar = series.shift(lookback + 1); right_bar = series.shift(lookback - 1)
        if is_high: flags = (shifted > left_bar) & (shifted > right_bar)
        else:       flags = (shifted < left_bar) & (shifted < right_bar)
        return flags.to_numpy()

    def iou_1d(self, bot_a, top_a, bot_b, top_b) -> float:
        intersect_top = min(top_a, top_b); intersect_bot = max(bot_a, bot_b)
        if intersect_top <= intersect_bot: return 0.0
        return (intersect_top - intersect_bot) / (max(top_a, top_b) - min(bot_a, bot_b))

    def is_duplicate_spatial(self, new_ob, queue) -> bool:
        return any(self.iou_1d(new_ob['bot'], new_ob['top'], ob['bot'], ob['top']) > self.iou_threshold for ob in queue)

    def create_ob(self, origin_idx, current_idx, is_high, highs, lows, closes, atrs):
        ...
        if len(phantom_closes) > 0:
            if is_high and (phantom_closes > top).any(): return None
            if not is_high and (phantom_closes < bot).any(): return None
        
        touches = int(np.sum((phantom_lows <= top) & (phantom_highs >= bot))) if len(phantom_closes) > 0 else 0
        return {'idx': origin_idx, 'top': top, 'bot': bot, 'touches': touches}

    def promote_or_create(self, origin_idx, current_idx, is_high, internal_q, swing_q, highs, lows, closes, atrs):
    ...
    def generate_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out_swg_supp = np.full(n, np.nan); out_swg_res = np.full(n, np.nan)
        out_int_supp = np.full(n, np.nan); out_int_res = np.full(n, np.nan)
        out_swg_supp_touches = np.zeros(n, dtype=np.float32); out_swg_res_touches = np.zeros(n, dtype=np.float32)
        mask_swg_supp = np.zeros(n, dtype=bool); mask_swg_res = np.zeros(n, dtype=bool)

        for i in range(self.max_window, n):
            curr_h, curr_l, curr_c = highs[i], lows[i], closes[i]
            if np.isnan(curr_h) or np.isnan(curr_l) or np.isnan(curr_c): continue
                
            if swg_ph[i]: self.promote_or_create(i - self.swg_lb, i, True, int_bear, swg_bear, highs, lows, closes, atrs)
            if swg_pl[i]: self.promote_or_create(i - self.swg_lb, i, False, int_bull, swg_bull, highs, lows, closes, atrs)

            if int_ph[i]:
                new_ob = self.create_ob(i - self.int_lb, i, True, highs, lows, closes, atrs)
                if new_ob and not self.is_duplicate_spatial(new_ob, swg_bear) and not self.is_duplicate_spatial(new_ob, int_bear): int_bear.append(new_ob)
            if int_pl[i]:
                new_ob = self.create_ob(i - self.int_lb, i, False, highs, lows, closes, atrs)
                if new_ob and not self.is_duplicate_spatial(new_ob, swg_bull) and not self.is_duplicate_spatial(new_ob, int_bull): int_bull.append(new_ob)
            ...
            for q, key, is_bull in [(swg_bull, 'bot', True), (int_bull, 'bot', True), (swg_bear, 'top', False), (int_bear, 'top', False)]:
                to_evict = [ob for ob in q if curr_c < ob[key]] if is_bull else [ob for ob in q if curr_c > ob[key]]
                for ob in to_evict: q.remove(ob)
            ...
            if swg_bear:
                ...
                mask_swg_res[i] = True
            if int_bull: out_int_supp[i] = max(int_bull, key=lambda x: x['top'])['top']
            if int_bear: out_int_res[i] = min(int_bear, key=lambda x: x['bot'])['bot']
...
def calculate_features(
    ...
    bars_per_day: int = 13,                # NEW: Expected 30-min bars in a day
    ...
    out["feat_vol_asymmetry"] = directional_vol_asymmetry(log_ret=log_ret, window=vol_asym_window)
